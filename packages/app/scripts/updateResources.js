const fs = require('fs')
const runCordovaRes = require('cordova-res') // https://github.com/ionic-team/cordova-res

const copyResourcesToApp = (maracujaDomainId) => {
  const SOURCE_IOS_ICON = `resources/apps/${maracujaDomainId}/ios/icon/`
  const SOURCE_IOS_SPLASH = `resources/apps/${maracujaDomainId}/ios/splash/`
  ...
  const SOURCE_ANDROID_ICON = `resources/apps/${maracujaDomainId}/android/icon/`
  const SOURCE_ANDROID_SPLASH = `resources/apps/${maracujaDomainId}/android/splash/`
...
exports = module.exports = async ({ maracujaDomainId, generateIconAndSplash }) => {
  const options = {
  // directory: '/resources/',
    resourcesDirectory: `./resources/apps/${maracujaDomainId}/`,
    // logstream: process.stdout, // Any WritableStream
    platforms: {
      android: {
        icon: { sources: [`resources/apps/${maracujaDomainId}/icon-android.png`] },
        splash: { sources: [`resources/apps/${maracujaDomainId}/splash.png`] }
      },
      ios: {
        icon: { sources: [`resources/apps/${maracujaDomainId}/icon.png`] },
        splash: { sources: [`resources/apps/${maracujaDomainId}/splash.png`] }
      }
  ...
  if (generateIconAndSplash) {
    console.log('icon and splash are generating...')
    await runCordovaRes(options)
  }
  console.log('icon and splash generated')
  copyResourcesToApp(maracujaDomainId)
  console.log('icon and splash copied in apps')
}
